...
class 扫描模式:
    """三种扫描模式"""
    简单 = "简单"      # 快速扫，主要用于去重，不做AI识别
    快速 = "快速"      # 快速扫，用于去重，兼做AI识别（少量采样）
    深度 = "深度"      # 深度扫，专为AI识别优化（密集采样）
...
class 统一扫描器:
    ...
    def __init__(self, 并行数: int = 4, 模式: str = 扫描模式.快速):
        self.并行数 = 并行数
        self.模式 = 模式
        self.配置 = 扫描配置[模式]
        self.人脸检测器 = None
        ...
        if 有人脸识别 and self.配置["人脸检测"]:
            try:
                self.人脸检测器 = insightface.app.FaceAnalysis()
                self.人脸检测器.prepare(ctx_id=-1, det_size=(640, 640))
                print(f"✓ 人脸识别模型加载成功（{self.配置['名称']}）")
            except Exception as e:
                print(f"✗ 人脸识别模型加载失败: {e}")
    
    def 扫描(self, 路径列表: List[str], 增量: bool = True, 
             硬盘保护: bool = True, 批量大小GB: int = 100, 休息秒数: int = 10) -> Dict:
        """
        统一扫描入口
        参数:
            路径列表: 要扫描的文件夹路径列表
            增量: 是否跳过已存在的视频
            硬盘保护: 是否启用硬盘保护（每100GB暂停）
            批量大小GB: 硬盘保护的批量大小（GB）
            休息秒数: 每批处理后的休息时间（秒）
        返回: {'去重指纹数': x, '人物记录数': y}
        """
        print(f"\n{'='*60}")
        print(f"🎬 开始扫描 - {self.配置['名称']}")
        print(f"   {self.配置['描述']}")
        print(f"   采样间隔: {self.配置['采样间隔']}秒" if self.配置['采样间隔'] > 0 else "   不进行AI采样")
        if 硬盘保护:
            print(f"   硬盘保护: 每{批量大小GB}GB休息{休息秒数}秒")
        print(f"{'='*60}")
        ...
        # 计算总大小
        总大小 = sum(os.path.getsize(p) for p in 视频文件 if os.path.exists(p))
        print(f"\n发现 {len(视频文件)} 个视频，总计 {总大小/(1024**3):.1f} GB")
        ...
        if 增量:
            ...
            跳过大小 = sum(os.path.getsize(p) for p in 视频文件 if p in 已存在 and os.path.exists(p))
            print(f"需要处理: {len(待处理)} 个（跳过 {len(视频文件) - len(待处理)} 个已存在，{跳过大小/(1024**3):.1f} GB）")
        else:
            待处理 = 视频文件
        ...
        # 按大小排序（先处理小的，快速看到进度）
        待处理.sort(key=lambda x: os.path.getsize(x))
        
        # 设置硬盘保护参数
        批量大小 = 批量大小GB * 1024 * 1024 * 1024
        self.已处理大小 = 0
        self.当前批次数 = 0
        ...
        for i, 路径 in enumerate(tqdm(待处理, desc="扫描进度")):
            文件大小 = os.path.getsize(路径)
            
            # 硬盘保护检查
            if 硬盘保护 and self.已处理大小 + 文件大小 > 批量大小:
                self._硬盘休息(休息秒数)
                self.已处理大小 = 0
                self.当前批次数 += 1
        ...
        处理大小GB = sum(os.path.getsize(p) for p in 待处理 if os.path.exists(p)) / (1024**3)
        
        print(f"\n{'='*60}")
        print(f"✅ 扫描完成")
        print(f"   处理视频: {len(待处理)} 个")
        print(f"   处理数据: {处理大小GB:.1f} GB")
        print(f"   生成指纹: {len(去重指纹列表)} 个")
        print(f"   人物记录: {len(人物记录列表)} 条")
        print(f"   休息次数: {self.当前批次数} 次")
        print(f"   总耗时: {耗时/60:.1f} 分钟")
        print(f"{'='*60}")
    ...
    def _硬盘休息(self, 休息秒数: int = 10):
        """硬盘保护：休息一段时间"""
        import time
        print(f"\n💤 硬盘保护：已处理 {硬盘保护配置['批量大小']/(1024**3):.0f} GB，休息 {休息秒数} 秒...")
        for i in range(休息秒数, 0, -1):
            print(f"   剩余 {i} 秒...", end='\r')
            time.sleep(1)
        print("   继续扫描！          ")
    
    def _保存中间结果(self, 去重指纹列表, 人物记录列表):
        """保存中间结果，防止数据丢失"""
        if not 去重指纹列表 and not 人物记录列表:
            return
    ...
    def 处理单个视频(self, 路径: str) -> Optional[Tuple[视频指纹, List[人物出现记录]]]:
        ...
        try:
            ...
            # 识别人物（根据扫描模式决定是否执行）
            人物列表 = []
            if 有人脸识别 and self.人脸检测器 and self.配置["人脸检测"]:
                人物列表 = self.识别人物(路径)
    ...
    def 识别人物(self, 视频路径: str) -> List[人物出现记录]:
        ...
        # 使用扫描模式配置的采样参数
        采样间隔 = self.配置.get("采样间隔", 5)
        最大采样数 = self.配置.get("最大采样数", 20)
        
        if 采样间隔 <= 0 or 最大采样数 <= 0:
            视频.release()
            return []
        
        采样次数 = min(int(时长 / 采样间隔), 最大采样数)
    ...
    def 匹配已知人物(self, 人脸特征) -> Optional[Tuple[str, float]]:
        """
        将检测到的人脸与已知人物匹配
        返回: (人物名, 置信度) 或 None
        """
    ...
    def 计算余弦相似度(self, 向量1, 向量2) -> float:
        """计算两个向量的余弦相似度"""
    ...
    def 保存人脸截图(self, 帧, 人脸框, 人物名: str, 视频路径: str, 时间: float) -> Optional[str]:
        """保存人脸截图"""
        try:
            # 创建人物截图目录
            人物目录 = 截图目录 / 人物名
...
def 主函数():
    ...
    解析器 = argparse.ArgumentParser(
        ...
        epilog="""
扫描模式说明:
  --模式 简单      简单模式: 仅去重哈希, 不做AI识别 (最快)
  --模式 快速      快速模式: 去重+AI识别, 10秒采样间隔 (推荐)
  --模式 深度      深度模式: 去重+AI识别, 3秒采样间隔 (最准)

硬盘保护说明:
  --硬盘保护 100   每处理100GB休息一次 (默认)
  --硬盘保护 0     关闭硬盘保护
  --休息 10        每次休息10秒 (默认)
  --休息 30        每次休息30秒 (适合发热大的硬盘)

使用示例:
  # 基础用法
  视频管家 初始化                    # 创建数据库和目录
  视频管家 扫描 /视频文件夹           # 快速模式扫描
  视频管家 找重复                    # 查找重复视频
  
  # 高级扫描
  视频管家 扫描 /视频文件夹 --模式 简单              # 仅去重, 不做人脸识别
  视频管家 扫描 /视频文件夹 --模式 深度              # 深度AI识别
  视频管家 扫描 /视频文件夹 --硬盘保护 50            # 每50GB休息一次
  视频管家 扫描 /视频文件夹 --硬盘保护 50 --休息 30  # 每50GB休息30秒
  视频管家 扫描 /视频文件夹 --模式 简单 --硬盘保护 0 # 简单模式, 不休息
  ...
  视频管家 扫描 /视频文件夹 --增量    # 只扫描新增的视频
    ...
    解析器.add_argument('--模式', choices=['简单', '快速', '深度'], 
                       default='快速', help='扫描模式 (默认: 快速)')
    解析器.add_argument('--硬盘保护', type=int, default=100, metavar='GB',
                       help='每处理N GB数据后休息 (默认: 100, 0=禁用)')
    解析器.add_argument('--休息', type=int, default=10, metavar='秒',
                       help='硬盘保护时的休息时间 (默认: 10秒)')
    ...
    if 参数.命令 == '初始化':
        ...
        # 硬盘保护
        启用硬盘保护 = 参数.硬盘保护 > 0
        批量大小 = 参数.硬盘保护 if 启用硬盘保护 else 100
        休息秒数 = 参数.休息 if 启用硬盘保护 else 0
        
        扫描器 = 统一扫描器(模式=选定模式)
        结果 = 扫描器.扫描([参数.路径], 增量=参数.增量, 
                          硬盘保护=启用硬盘保护, 批量大小GB=批量大小, 休息秒数=休息秒数)
        
        print(f"\n📊 扫描结果:")
        print(f"   处理数据: {结果.get('处理大小GB', 0):.1f} GB")
        print(f"   生成指纹: {结果['去重指纹数']} 个")
        print(f"   人物记录: {结果['人物记录数']} 条")
        if 结果.get('休息次数', 0) > 0:
            print(f"   硬盘休息: {结果['休息次数']} 次")
